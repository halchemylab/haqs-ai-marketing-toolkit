...
Keep one editable text file per brand in `brands/`, such as `brands/default.txt`.
See [the profile template](brands/README.md) for fields and examples. When client
...
```powershell
haqs-create --scope complete --job-type campaign --brief brief.json --brands default
```
...
`--brands default` refreshes it, and `--brands general` uses the global brand voice.
